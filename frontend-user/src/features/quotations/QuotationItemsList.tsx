...
export function QuotationItemsList({ items, compact = false }: QuotationItemsListProps) {
  return (
        ...
        <div key={item.id ?? `${item.productId}-${idx}`} className="flex gap-3 text-sm">
          {item.productImage ? (
            <img
              ...
              className={compact ? 'size-10 rounded object-cover' : 'size-12 rounded object-cover'}
            />
          ) : (
            <div className={compact ? 'size-10 rounded bg-muted' : 'size-12 rounded bg-muted'} />
          )}
